...
#     H = zeros((2, M))
#     H[0, 1:] = m[1:] * (M - m[1:]) / 2
#     H[1, :] = ((M - 1 - 2 * m) / 2)**2 * cos(2 * pi * width)
...
#             else:
#                 s = cp.fft.rfft(windows[0])
#                 shift = -(1 - 1./M) * arange(1, M//2 + 1)
#                 s[1:] *= 2 * cp.exp(-1j * pi * shift)
