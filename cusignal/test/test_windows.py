...
def test_general_cosine(num_samps):
    ...
    gpu_window = cp.asnumpy(cusignal.windows.general_cosine(num_samps, HFT90D, sym=False))
...
def test_general_hamming(num_samps, alpha):
    ...
    gpu_window = cp.asnumpy(cusignal.windows.general_hamming(num_samps, alpha, sym=True))
    assert array_equal(cpu_window, gpu_window)
