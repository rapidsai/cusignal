...
class TestCaCfar:
    ...
    class TestOneD:
        def expected(self, length, guard_cells, reference_cells):
            ...
            out[guard_cells+reference_cells:
                -guard_cells-reference_cells] = cp.ones(length-2*guard_cells
                                                        -2*reference_cells)
            return(alpha*out)
    ...
    @pytest.mark.parametrize("shape, gc, rc",
                             [((10,10), (1,1), (2,2)), ((10,100), (1,10), (2,20))])
    class TestTwoD:
        def expected(self, shape, gc, rc):
            ...
            out[gc[0]+rc[0]:-gc[0]-rc[0], gc[1]+rc[1]:-gc[1]-rc[1]] = \
                cp.ones((shape[0]-2*gc[0]-2*rc[0], shape[1]-2*gc[1]-2*rc[1]))
            return(alpha*out)
