...
class TestCaCfar:
    ...
    @pytest.mark.parametrize(
        "length, guard_cells, reference_cells", [(100, 1, 5), (11, 2, 3), (100, 10, 20)]
    )
    class TestOneD:
        def expected(self, length, guard_cells, reference_cells):
            ...
            out[
                guard_cells + reference_cells : -guard_cells - reference_cells
            ] = cp.ones(length - 2 * guard_cells - 2 * reference_cells)
            return alpha * out
    ...
    @pytest.mark.parametrize(
        "length, gc, rc", [(1, 1, 1), (10, 2, 3), (10, 0, 5), (10, 5, 0)]
    )
    ...
    class TestTwoD:
        def expected(self, shape, gc, rc):
            ...
            alpha = cfar_alpha(0.001, N)
            out[
                gc[0] + rc[0] : -gc[0] - rc[0], gc[1] + rc[1] : -gc[1] - rc[1]
            ] = cp.ones(
                (shape[0] - 2 * gc[0] - 2 * rc[0], shape[1] - 2 * gc[1] - 2 * rc[1])
            )
            return alpha * out
    ...
    @pytest.mark.parametrize(
        ...
        [
            (10, 1, 1, (6,)),
            (100, 10, 20, (34, 67)),
            ((100, 200), (5, 10), (10, 20), [(31, 45), (50, 111)]),
        ],
    )
    class TestDetection:
